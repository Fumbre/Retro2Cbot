#include "lineInterpreter.h"

// --- bitmask tables ---
const uint8_t centerPatterns[]      = { 0b00011000, 0b00111100 };
const uint8_t slightLeftPatterns[]  = { 0b00110000, 0b01110000, 0b00111000 };
const uint8_t slightRightPatterns[] = { 0b00001100, 0b00001110, 0b00011100 };
const uint8_t hardLeftPatterns[]    = { 0b11100000, 0b11000000, 0b10000000 };
const uint8_t hardRightPatterns[]   = { 0b00000111, 0b00000011, 0b00000001 };
const uint8_t allWhite[]            = { 0b00000000, }; 
const uint8_t allBlack[]            = { 0b11111111, };
const uint8_t leftTurn[]            = { 0b11111000, 0b11110000, 0b11111100, };
const uint8_t rightTurn[]           = { 0b00011111, 0b00001111, 0b00111111, };
const uint8_t test[]                = { 0b10000001, 0b11000011, 0b11100111, 0b10000011, 
                                        0b11000001, 0b11100011, 0b11000111, 0b11100001, 0b10000111};



bool LineInterpreter::match(uint8_t mask, const uint8_t* patterns, int count) {
    for (int i = 0; i < count; i++) {
        if (mask == patterns[i]) return true;
    }
    return false;
}

LineState LineInterpreter::interpret(uint8_t mask) {

    if (match(mask, centerPatterns, sizeof(centerPatterns))) return CENTER;
    if (match(mask, slightLeftPatterns, sizeof(slightLeftPatterns))) return SLIGHT_LEFT;
    if (match(mask, slightRightPatterns, sizeof(slightRightPatterns))) return SLIGHT_RIGHT;
    if (match(mask, hardLeftPatterns, sizeof(hardLeftPatterns))) return HARD_LEFT;
    if (match(mask, hardRightPatterns, sizeof(hardRightPatterns))) return HARD_RIGHT;

    if (match(mask, leftTurn, sizeof(leftTurn))) return LEFT_TURN;
    if (match(mask, rightTurn, sizeof(rightTurn))) return RIGHT_TURN;

    if (match(mask, allWhite, sizeof(allWhite))) return ALL_WHITE;
    if (match(mask, allBlack, sizeof(allBlack))) return ALL_BLACK;

    if (match(mask, test, sizeof(test))) return TEST_PATTERN;

    return CENTER;
}
