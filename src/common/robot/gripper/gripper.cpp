...
void gripper(int pulseMicroseconds) {
    ...
    if (pulseMicroseconds > 0) {                        // If a new pulse width is provided (must be > 0)
    ...
    if (now >= nextPulseTime) {                         // Only generate a pulse if 20 ms have passed
